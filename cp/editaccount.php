<?php
	...
	include("config.php");
	if(isset($_SESSION['u']) and isset($_SESSION['p']) and isset($_REQUEST['uid']) )
	{
		$user = $_SESSION['u'];
		$pass = $_SESSION['p'];
		$uid = $_REQUEST['uid'];
	...
	$account = mysql_query("Select * from users where id ='$uid'")or die(mysql_error());
...
<tr>
	...
	<td style="border-bottom:1px solid #808080; border-top:2px solid #000000; " width="492">
		<b><font face="Albany" color="#000080">Edit Account</font></b></td>
...
<table align = center width = 96% cellpadding = 0 cellspacing =0 border = 0 bgcolor=#FFFFFF>
				<tr>
					<td>
						...
						<tr>
							...
							<font size="2" color="#FFFFFF" face="Arial Narrow">&nbsp;Acount</font></td>
				...
				<tr>
					...
					<input type =hidden name =uid value ='<?php print $account['id'];?>'>
					...
					<tr>
						...
						<td height="29"><input type =text name = uname size="28" value ='<?php print $account['name'];?>'> 
					...
					<tr>
						...
						*</font> </td>
						<td height="28"><input type =text name = dd size="28" value ='<?php print $account['id'];?>' disabled> 
						<?php 
					...
					<tr>
						...
						*</font> </td>
						<td height="27"><input type =password name = pas1 size="28"  value ='<?php print $account['pass'];?>'>
						<?php 
					...
					<tr>
						...
						<td><input type = password name = pas2 size="28"  value ='<?php print $account['pass'];?>'></td>
					</tr>
					<tr>
						<td width="110"></td>
						<td>&nbsp;</td>
					...
					<tr>
						...
						<?php
							...
							{
								$item = $mainLinks[$i];
								print "	<li><input style ='margin-left:-3px' type ='checkbox' name = 'id[]' value='$item' id ='$item'";
								if($account[$item]=='t') 
									print "checked";
								print "><b><label for ='$item' >$item</label></b></li>\n";
							}
						?>
						</ol>
						</td>
					</tr>
					<tr><td colspan = 2 align =center> <input type = submit name =changeaccount value = Change>&nbsp; <input type = reset ></td></tr>
					</table>	
